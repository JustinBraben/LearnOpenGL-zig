...
pub fn main() !void {
    ...
    var shader_program: Shader = try Shader.create(arena, "src/1.getting_started/4.4.textures_exercise2/4.4.texture.vs", "src/1.getting_started/4.4.textures_exercise2/4.4.texture.fs");
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE); // note that we set the container wrapping method to GL_CLAMP_TO_EDGE
    ...
    // set the texture1 wrapping parameters
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT); // we want to repeat the awesomeface pattern so we kept it at GL_REPEAT
