...
pub fn main() !void {
    ...
    // Fragment shader
    var fragmentShaderOrange: c_uint = undefined;
    fragmentShaderOrange = gl.createShader(gl.FRAGMENT_SHADER);
    defer gl.deleteShader(fragmentShaderOrange);
    gl.shaderSource(fragmentShaderOrange, 1, @as([*c]const [*c]const u8, @ptrCast(&fragmentShaderSource_1)), 0);
    gl.compileShader(fragmentShaderOrange);
    gl.getShaderiv(fragmentShaderOrange, gl.COMPILE_STATUS, &success);
    if (success == 0) {
        gl.getShaderInfoLog(fragmentShaderOrange, 512, 0, &infoLog);
        std.log.err("{s}", .{infoLog});
    }

    var fragmentShaderYellow: c_uint = undefined;
    fragmentShaderYellow = gl.createShader(gl.FRAGMENT_SHADER);
    defer gl.deleteShader(fragmentShaderYellow);
    gl.shaderSource(fragmentShaderYellow, 1, @as([*c]const [*c]const u8, @ptrCast(&fragmentShaderSource_2)), 0);
    gl.compileShader(fragmentShaderYellow);
    gl.getShaderiv(fragmentShaderYellow, gl.COMPILE_STATUS, &success);
    if (success == 0) {
        gl.getShaderInfoLog(fragmentShaderYellow, 512, 0, &infoLog);
        std.log.err("{s}", .{infoLog});
    }

    // create a program object
    var shaderProgramOrange: c_uint = undefined;
    var shaderProgramYellow: c_uint = undefined;
    ...
    if (success == 0) {
        gl.getProgramInfoLog(shaderProgramOrange, 512, 0, &infoLog);
        std.log.err("{s}", .{infoLog});
    }
    ...
    if (success == 0) {
        gl.getProgramInfoLog(shaderProgramYellow, 512, 0, &infoLog);
        std.log.err("{s}", .{infoLog});
    }
