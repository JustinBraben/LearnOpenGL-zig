...
pub fn main() !void {
    ...
    var shader_program: Shader = try Shader.create(arena, "src/1.getting_started/7.1.camera_circle/7.1.camera.vs", "src/1.getting_started/7.1.camera_circle/7.1.camera.fs");
    ...
    while (!window.shouldClose()) {
        ...
        const camX = math.sin(@as(f32, @floatCast(glfw.getTime()))) * radius;
        const camY = math.cos(@as(f32, @floatCast(glfw.getTime()))) * radius;
        viewM = zm.lookAtRh(zm.f32x4(camX, 0.0, camY, 1.0), zm.f32x4(0.0, 0.0, 0.0, 1.0), zm.f32x4(0.0, 1.0, 0.0, 1.0));
        ...
        for (cube_positions, 0..) |cube_position, i| {
            ...
            const cube_rot = zm.matFromAxisAngle(
                ...
                math.degreesToRadians(@as(f32, @floatCast(glfw.getTime())) * 55.0 * rotation_direction),
            );
