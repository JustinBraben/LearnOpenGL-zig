...
pub fn main() !void {
    ...
    var shader = try Shader.create(
        arena, 
        "src/5.advanced_lighting/1.advanced_lighting/1.advanced_lighting.vs", 
    ...
    try loadTexture(floor_path, &floor_texture);
    ...
    const lightPos = [_]f32{0.0, 0.0, 0.0};
...
fn processInput(window: *glfw.Window, deltaTime: f32) void {
    ...
    if (window.getKey(.b) == .press and !blinnKeyPressed) {
        blinn = !blinn;
        blinnKeyPressed = true;
    }
    if (window.getKey(.b) == .release) {
...
fn loadTexture(path: [:0]const u8, textureID: *c_uint) !void {
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, wrapping); // set texture wrapping to GL_REPEAT (default wrapping method)
